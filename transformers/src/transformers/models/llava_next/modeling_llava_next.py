...
class LlavaNextForConditionalGeneration(LlavaNextPreTrainedModel):
    def __init__(self, config: LlavaNextConfig):
        ...
        self.selected_tokens_mask = None
    ...
    def add_spatial_tokens(self, masks, num_spatial_tokens): # for spatial baseline
    ...
    def HiRED_token_selection(self,top_layer_attn,bottom_layer_attn, num_patches, full_image_patch, num_vision_tokens, token_budget_rate,alpha,image_sizes):
        ...
        # token_budget_rate: float, how much we want to include from full image
        # alpha: float, how much we want to include from sub-images
        ...
        for idx, top_attn, bottom_attn in zip(range(len(num_patches)), top_layer_attn, bottom_layer_attn):
            ...
            num_total_budget = int(num_patch * num_vision_tokens * token_budget_rate)
            full_img_budget = min(int(num_total_budget * alpha), num_vision_tokens)
    ...
    def pack_HiRED_image_features(self, image_features, image_sizes, image_newline=None, image_features_masks=None):
        """
        Reshape, unpad and then pack each image_feature into a single image_features tensor containing all visual vectors.
        Apply the same operation to the image_features_masks. Then, apply the masks to reduce the number of visual vectors.
        Finally, concatenate all image_features into a single tensor.
        Args:
    ...
    def forward(
        ...
        if inputs_embeds is None:
            ...
            if pixel_values is not None and input_ids.shape[1] != 1 and pixel_values.size(0) > 0:
                ...
                # get HiRED config params
                if self.config.hired_config is not None:
                    token_budget_rate = self.config.hired_config.get("token_budget_rate", 1.0)
                    alpha = self.config.hired_config.get("alpha", 1.0)
                    spatial_budget = self.config.hired_config.get("spatial_budget", 0.0)
                vit_top_layer = 0
                vit_bottom_layer = 22
                num_of_ViT_attention_heads = 16

                # set hooks to get ViT attention scores
                ...
                if token_budget_rate < 1.0: #  token dropping is activated if token_budget_rate < 1.0
                    selected_tokens_mask = self.HiRED_token_selection(
                ...
                image_features, feature_lens = self.pack_HiRED_image_features(
                    image_features, 
                    image_sizes,
                    image_newline=None, # None or self.image_newline
                    image_features_masks=selected_tokens_mask
                )

                print(f"HiRED visual token count: {image_features.shape}")
                self.selected_tokens_mask = selected_tokens_mask # for viewing selected tokens
                self.pixel_values = pixel_values # for viewing pixel values
